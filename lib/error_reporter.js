  ...
  function startPromptError(originalHandler, e, fatal, stackLines) {
    ...
    [
      ...
      {text: 'Show Stack Trace', onPress: () => {
        Alert.alert("App Error", e.message + "\n\n========\nStack Trace:\n========\n" + stackLines,
        [
          {text: 'Close', style: 'cancel', onPress: () => {runOriginalHandler(originalHandler, e, fatal);} },
          {text: 'Report Email to developer', onPress: () => { reportError(stackLines, () => { runOriginalHandler(originalHandler, e, fatal); } ); } },
        ]);
      }},
    ]);
  }
  ...
  function errorHandler(e, fatal) {
    ...
    if (__DEV__) {
      ...
      .catch(error => {
        ...
        runOriginalHandler(originalHandler, e, fatal);
      });
    } else {
